...
/scripts
  generate-credentials.ts      // CLI: pnpm run generate-creds
...
### Prerequisites
- Active Polymarket.com account
- Polygon wallet with private key access

### Step 1: Export Private Key from Polymarket UI
1. Log into [Polymarket.com](https://polymarket.com)
2. Navigate to **Cash** section
3. Click **3 dots** menu
4. Select **Export Private Key**
5. Copy the private key (remove `0x` prefix if present)
6. Store securely - this key controls your wallet funds

### Step 2: Generate API Credentials (Optional - Required for Phase 3+)
...
This script will:
1. Prompt for your private key (or read from `.env.local`)
2. Connect to Polymarket CLOB API
3. Generate deterministic API credentials
4. Output credentials to add to `.env.local`
...
async function generateCredentials() {
  ...
  // Generate API credentials (deterministic from signature)
